...
function App() {
    ...
    const renderizarVista = () => {
        switch (vistaActiva) {
            ...
            case 'Comentarios pendientes':
                return <ComentariosPendientes / > ; //renderizaso o llamada a comentarios pendientes
            case 'Comentarios clasificados':
                return <div className = "p-8" > Contenido de Comentarios clasificados < /div>;
            case 'Historial de cambios':
                return <HistorialDeCambios / > ; // Llama al archivo HistorialCambios
            case 'Configuración':
    ...
    return ( <
        div className = "relative flex bg-gray-50" > { /* Barra lateral que recibe el estado de vista activa y la función para alternar notificaciones */ } <
        BarraLateral vistaActiva = { vistaActiva }
        cambiarVistaActiva = { cambiarVistaActiva }
        alternarNotificaciones = { alternarNotificaciones }
        />

        <
        div className = "flex-1" > { /* Mostramos la barra de notificaciones si mostrarNotificaciones es true */ } { mostrarNotificaciones && < BarraDeNotificaciones / > }

        { /* Renderizamos la vista según la opción seleccionada */ } { renderizarVista() } <
        /div> < /
        div >
    );
}
