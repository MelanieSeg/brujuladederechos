...
export default function ComentariosPendientes() {
    ...
    return ( <
        div className = "p-8" > { /* Título con mayor margen arriba y abajo, más grande y en negrita */ } <
...

        {
            mostrarSelectorFecha && ( <
                ...
                value = { fechaDesde }
                onChange = {
                    (e) => setFechaDesde(e.target.value)
                }
                ...
                value = { fechaHasta }
                onChange = {
                    (e) => setFechaHasta(e.target.value)
                }
                className = "border p-1 rounded ml-1 bg-white" /
                >
                <
                /label> <
                button onClick = { filtrarComentarios }
        ...
        tbody > {
            (comentariosFiltrados.length > 0 ? comentariosFiltrados : comentarios).map((comentario, index) => ( <
                ...
                td className = "p-2" >
                <
                button className = "bg-white text-gray-800 border border-gray-300 py-1 px-4 rounded" > Clasificar < /button> < /
                td > <
                /tr>
            ))
        } <
